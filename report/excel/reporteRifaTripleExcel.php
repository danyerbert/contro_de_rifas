...
switch ($metodoPago) {
    case 4:
            ...
            $sqlRifaTriple = "SELECT m.numero, m.monto_total, m.nombre_comprador, m.cedula, m.cantidad_pago, m.referencia_pm, p.metodo, v.nombre FROM registro_numero_triple_500 AS m 
...
$resultado = $mysqli->query($sqlRifaTriple);
...
$hoja_excel->setCellValue('E2', 'Cantidad de pago');
$hoja_excel->getColumnDimension('F')->setWidth(20);
$hoja_excel->setCellValue('F2', 'Referencia');
...
while ($row = $resultado->fetch_assoc()) {
    ...
    $hoja_excel->setCellValue('E'.$fila, $row['cantidad_pago']);
    $hoja_excel->setCellValue('F'.$fila, $row['referencia_pm']);
    $fila++;
}
